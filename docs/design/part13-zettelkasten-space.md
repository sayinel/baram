...
| 노트 ID | 타임스탬프 ID + 제목 (파일명 `202607051530 원자적 노트.md`) |
| 링크 형식 | **B안** — 저장은 `[[ID]]`(target=ID만), WYSIWYG는 ID→현재 제목 **live 렌더** |
| 디스크 구조 | `inbox/` + `notes/` |
...
- **New note from selection** — 선택 텍스트를 새 permanent 노트로 추출하고,
  **원위치에 그 노트로의 `[[ID]]` 링크를 삽입**(프로그램이 ID 직접 주입).
  Zettelkasten의 핵심 동작.
...
### 저장 형식 (B안): `[[ID]]` + WYSIWYG live 제목

- **저장/원문**: `[[202607051530]]` — target=ID만, display 없음. 소스 모드·`.md`
  원문에서만 ID가 보이며, WYSIWYG 편집·읽기 뷰에선 안 보인다.
- **WYSIWYG 렌더**: `WikilinkView`(NodeView)가 ID로 **현재 제목을 live 조회**해
  pill로 표시. 대상 노트 제목을 바꾸면 이 노트를 링크한 **모든 곳의 표시가 즉시 최신**.
- **리네임 안정성**: target이 ID라 제목/파일명을 바꿔도 링크가 안 깨지고, vault
  전역 링크 refactor도 불필요.
- wikilink 노드는 이미 `[[target|display]]`, `#heading`, `#^blockId`,
  `alias::target`(cross-vault §87)를 지원(§28) — B는 그중 target만 쓴다.

### 링크 작성 경로 (두 가지)

- **① 자동완성 (기본 경로):** `[[` → `wikilink-suggest` 팝업. **제목/별칭으로 퍼지
  검색**, 선택 시 `target=ID`를 삽입(사용자는 ID를 타이핑/열람하지 않음). 기존
  `wikilink-suggest.ts`/`WikilinkMenu`(현재 `target=파일명` 삽입)를 **제목 검색 +
  ID 삽입**으로 수정.
- **② 수동 타이핑 → B2 eager 정규화:** 사용자가 `[[제목]]`을 직접 닫으면 wikilink
  노드 생성 시(InputRule·붙여넣기 포함) 그 제목이 **기존 노트와 유일 매칭되면
  즉시 target을 그 노트의 ID로 치환**. 최종 저장은 항상 `[[ID]]`.
  - **중복 제목**: 유일 매칭 실패 → 자동완성으로 유도(폴더/ID/미리보기로 구분).
  - **오타/미존재**: dangling 링크로 두고 기존 `ghost-link` 처리 + 서제스터
    "create"로 즉석 생성.
...
1. target이 ID 패턴(`\d{12,14}`)이면 → 인덱스에서 해당 `id`의 노트로 해석
   (서브폴더 무관, `inbox/`·`notes/` 모두). **이것이 B의 주 경로.**
2. 아니면 파일명(확장자 제외) 정확/정규화 일치.
3. 아니면 title/alias 프론트매터 일치 (B2 정규화 전 링크·외부 편집 링크의 폴백).

이를 위해 링크 인덱서(`src-tauri/src/index/`)가 노트의 `id`(파일명 접두 또는
프론트매터)와 **현재 title**을 인덱싱한다. 이 id↔title↔path 인덱스를 WYSIWYG live
제목 렌더·B2 eager 정규화·리졸버가 **공유**한다. `resolve_target`은 vaultType에
따라 분기. 백링크 계산도 동일 리졸버를 쓰므로 이 확장으로 백링크가 자동 일관.

### export / 상호운용 주의

B는 원문이 `[[ID]]`라 **Baram 외부**(GitHub 미리보기·타 에디터·raw `.md`)에선
ID만 보인다. Baram의 export 파이프라인(HTML/Pandoc/Notion)은 **ID→현재 제목으로
해석**해 사람이 읽는 형태로 내보낸다. 순수 원문 상호운용을 중시하면 A(`[[ID|제목]]`,
자기설명적이나 라벨 stale)가 유리 — 이는 B의 수용된 트레이드오프.
...
- `index/normalizer.rs` + `index/`: §95의 ID 기반 리졸버 + **id↔title↔path 인덱싱**
  (WYSIWYG live 제목·B2 정규화가 공유).
...
빠른 캡처 ──► resolveCaptureTarget() ──► zettel/inbox/{id}.md  (fleeting)
                                              │  Promote
                                              ▼
① [[ 자동완성 ─► 제목 검색 ─► [[ID]] 삽입 ─┐   zettel/notes/{id} 제목.md  (permanent)
② 수동 [[제목]] ─► B2 정규화 ─► [[ID]] ────┤        │
                                          ▼        ▼
                       id↔title↔path 인덱스 ◄── 저장 시 인덱싱 ─► 백링크/그래프/검색(기존)
                          │   │
                          │   └─► WYSIWYG: ID→현재 제목 live 렌더 (pill)
                          └─► resolve_target(zettel): ID→파일 / 파일명 / title·alias
                                                      └─► export: ID→제목 치환
```
...
- **노트 생성**: 파일명/프론트매터, New-from-selection의 `[[ID]]` 삽입, 캡처 타깃 resolution.
- **링크(B안)**: 자동완성이 제목 검색→`target=ID` 삽입, 수동 `[[제목]]`의 B2 eager
  정규화(유일 매칭→ID / 중복→미정규화 / 미존재→dangling), WYSIWYG live 제목 렌더
  (대상 제목 rename 시 링크 표시 갱신), export의 ID→제목 치환.
...
- **P2 (이번)**: §92 레지스트리 + Journal 이관(A1–A3), §93–§99 Zettelkasten 코어
  (디스크 구조, ID, New Zettel / New-from-selection / Quick Capture, **B안 링크**
  = `[[ID]]` 저장 + WYSIWYG live 제목 + 자동완성/B2 정규화 + 리졸버 확장,
  inbox→promote, 기본 MOC, UI 배선, 캡처 이전).
...
- **R2 — 리졸버 확장 파급.** ID 기반 리졸버가 general vault의 기존 `[[정확일치]]`
  ...
  zettel vault에서만 활성.
- **R3 — B안 live 제목 의존.** WYSIWYG 표시가 id↔title 인덱스의 live 조회에 의존
  → 인덱스 미갱신/미스 시 pill이 ID로 표시될 수 있음. 완화: 인덱스 로드 전 폴백
  표시(파일명 축약) + 저장/리네임 시 인덱스 갱신. 대량 노트에서 조회 성능도 점검.
- **R4 — 외부 상호운용.** raw `.md`를 Baram 밖에서 보면 `[[ID]]`만 노출(제목 없음).
  완화: export 파이프라인에서 ID→제목 치환. 순수 원문 가독성은 A 대비 열위(수용된 트레이드오프).
- **O1 — 캡처 폴백 정책**: 제텔 스페이스 미존재 시 캡처 동작(비활성 vs 일기 폴백) — A3에서 확정.
- **O4 — 제목 새니타이저**: 제목 내 금지문자(`/ \ : * ? " < > |`)·길이·유니코드 정규화
  처리 규칙 — §94 파일명 생성 시 확정.
