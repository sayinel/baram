...
- [ ] **Step 6:** Memory guard: on keep-alive creation log `[Baram Perf] keepalive: +1 editor (N blocks)`; eviction logs too. Settings escape hatch NOT included (YAGNI) — threshold constant only.
- [ ] **Step 7:** Tests: keep-alive pool unit tests (threshold, LRU eviction destroys, close destroys); routing decision function extracted pure and tested (`resolveTabEditor(tab, pool) → "shared" | "keepalive"`). Hook-level mounting stays out of scope (jsdom impractical — same rationale as C2.4, do not fabricate).
- [ ] **Step 8:** Full suite + tsc. Commit `perf(§perf-large-file C3.5): keep large-document editor DOM alive across tab switches`.
