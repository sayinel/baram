...
전체 96개 기능 중 68개 완료 (71%), 28개 남음 (29%).

---

## M9 — Productivity Tools (9개 기능)

| 우선순위 | 기능 | 설명 | 비고 |
|---------|------|------|------|
| 높음 | **§50 Mermaid Diagrams (고도화)** | 11종 다이어그램, 소스 편집 + 라이브 프리뷰 | M7에서 기본 구현 완료, 고도화 |
| 높음 | **§51 Global Search (Cmd+Shift+F)** | tantivy 전문검색, regex, 파일/폴더 필터, Replace | tantivy 인프라 이미 있음 |
...
| 기능 | 문법 | 비고 |
|------|------|------|
| **Highlight** | `==text==` | README Upcoming에 명시 |
| **Subscript** | `~text~` | |
| **Superscript** | `^text^` | |
| **Table of Contents** | `[TOC]` / `/toc` | 동적 헤딩 목록, 실시간 갱신 |
...
1. **Global Search (§51)** — tantivy 백엔드 준비 완료, 핵심 생산성 기능
2. **Highlight / Subscript / Superscript** — 작은 범위, M9 전에 빠르게 처리 가능
3. **Git Basic (§57b)** — Phase 3 Git Advanced의 기반
