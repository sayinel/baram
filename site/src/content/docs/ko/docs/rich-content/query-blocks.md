...
태스크 기능이 켜져 있어야 합니다(**설정 › 태스크**). 꺼져 있으면 블록이 빈 목록을 보이는 대신 그렇다고 말합니다.
...
| `Tasks are turned off. Settings › Tasks.` | 태스크 기능이 꺼진 상태에서 `source: tasks` 를 썼습니다              |
