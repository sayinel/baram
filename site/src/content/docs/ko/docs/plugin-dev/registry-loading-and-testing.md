...
무엇이든 빌드되기 전에 거부가 둘 일어나고, 둘은 서로 다른 질문에 답합니다. **어느 디렉터리**가
나갈 수 있는가 — 워크플로가 기본 거부인 배포 allowlist를 들고 있어서, 태그를 잘못 쳐도 테스트
픽스처가 배포될 수 없고 예제를 추가한다고 배포 대상이 되지도 않습니다. **어느 티어**로 나가는가 —
같은 allowlist가 디렉터리마다 티어를 하나씩 지정하고, 매니페스트의 `trust`가 그 티어가 아니면
릴리스가 실패합니다. 마켓플레이스가 다른 동의 화면으로 보여 줄 항목을 배포하는 대신에 말입니다.

두 티어 모두 배포될 수 있지만, `trusted`는 기본값이 아니라 디렉터리마다 내리는 결정입니다.
Tiptap 확장을 기여하는 플러그인은 메인 렐름에서 돌고 따라서 구조적으로 `trusted`이므로(샌드박스
티어는 `tiptapExtensions`를 거부합니다), 그 티어를 부류째 거부하면 레지스트리가 에디터 플러그인을
아예 담을 수 없게 됩니다.
...
상수가 로컬 시드를 가리키면 **설정 → 플러그인**("둘러보기" 탭)을 여십시오 — 마운트될 때
`fetchRegistryIndex()`를 호출합니다. 레지스트리 응답은 24시간 캐시된다는 점을 유의하십시오.
**둘러보기**와 **업데이트** 탭에는 늘 쓸 수 있는 **새로고침** 버튼이 있어 캐시를 우회하고
