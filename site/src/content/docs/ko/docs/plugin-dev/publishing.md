...
때까지는 시드로 마켓플레이스 **UI**를 시험하고(목록, 권한·티어 배지, 레거시 상태, 새로 고침),
...
- 자리표시자 체크섬은 자동으로 채워지지 **않습니다**. 릴리스 워크플로는
  ...
  잊는 것은 이제 **신고되지만 막히지는 않습니다** — `validate-index.ts`가 `npm run lint`마다
...
5. 배포하려는 `RegistryIndex`에 `RegistryEntry`를 추가합니다. ⚠️ 오늘은 그것이 1군 레지스트리만을
   ...
   않습니다. 커뮤니티 제출이 열리기 전까지는 사용자가 **설정 → 플러그인**의 **개발자** 구역에서
   폴더를 불러오게 해 배포하십시오. (이 리포의 1군 플러그인은 항목을 손으로 넣지 않습니다 —
