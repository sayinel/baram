...
- **제거** — 플러그인을 내리고 그 개발 폴더를 잊습니다(디스크의 어떤 것도 지우지 않습니다).
...
남은 맨 `import`는 해석될 수 없습니다. 무언가를 external로 두면 플러그인이 해석 오류로
불려옵니다. (이 티어는 Tiptap을 아예 쓸 수 없습니다 — 확장은 메인 렐름의 ProseMirror 인스턴스에
