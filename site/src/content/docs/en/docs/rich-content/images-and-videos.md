...
An image written on the same line as text (`Logo: ![alt](image-url)`) is rendered like any other, but clicking it or pressing `Enter` on it does not open its markdown for inline editing — edit that markdown in source mode (`Cmd/Ctrl+/`) instead. Saving keeps the line as you wrote it.
