...
Only `[[…]]` links are collected into the link index, so only they appear in backlinks and the Graph View. A link or block reference written inside a code block, a code span, raw HTML, a formula or an image's alt text is an example, not a link: it is not indexed, and renaming a file or a block ID leaves it exactly as written.
