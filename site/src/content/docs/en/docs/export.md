...
Word and EPUB embed images, so Pandoc has to read the image files. Baram only lets it read images that live inside the vault or folder the note belongs to — the narrowest one you have open that contains it — and are referred to by a relative path (`![alt](img/diagram.png)`); it copies them next to the document it hands to Pandoc, so Pandoc never opens a path from your note directly. Everything else — an absolute path, a `file:` URL, a web URL, or a path that leads outside the vault — is replaced by the image's alt text, so a Markdown image never makes a network request during export. If a path that looks like it stays inside the vault turns out to lead outside it (a symbolic link, for example), the export stops and names the image rather than quietly leaving it out. An image file that cannot be found is replaced by its alt text as well, with a warning from Pandoc. An unsaved document has no folder for relative paths to start from, so its relative images are replaced too; the same goes for a file opened on its own rather than as part of a vault or folder — open its folder to embed its images. LaTeX and RST do not embed images — Pandoc writes the reference and opens no file — so for those formats image destinations are not filtered by this rule. Raw HTML in a note (`<img src="…">`, `<video>`, and the rest) is left out of Word and EPUB output: Word never rendered it, and in EPUB it could name files the same way an image does. Frontmatter keys that hand Pandoc a file, such as `cover-image` and `css`, are ignored for the same reason.
