...
- **Phase 3 (고급 기능)** — 진행 중
  ...
  - 🚢 **미발행** — `v0.7.4..main` 206커밋(#741 머지 시점). **입력 보조**(스펙 0056 — 스마트 구두점을 처음으로
    ...
    **크롬 숨기기와 포커스 화면구성**(§370 #728 — Writing 이 처음으로 사이드바를 실제로 닫는다,
    ...
    이 범위 안에서 생긴 24키 테마 팔레트 결함은 태그 전에 고쳤다(#741, §361)
    ‼️ **테마 레지스트리는 여전히 비어 있다** — 테마가 다이얼·크롬을 제안하는 매니페스트 필드와 설치 시
    대비 경고는 실린 채로 쓰이지 않는다. 테마 배포 사용자 문서는 0줄(외관 다이얼 절만 있다)
