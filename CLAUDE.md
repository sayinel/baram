...
- **Phase 3 (고급 기능)** — 진행 중
  ...
  - 🚢 **미발행** — `v0.7.3..main` 264커밋, 배포 경로 둘이 열린다: **테마 배포**(§355~§363 —
    모델 통일·CSS 위생 파이프라인·마켓플레이스·패키지 내보내기, #689·#697·#709)와 **플러그인이
    에디터를 확장**(§260 — `tiptapExtensions` 배선·`extensions` capability·trusted 티어 게시,
    #693·#694·#696·#701). 여기에 **외관 모델 기반**(§364·§366·§369 — 기본→테마→사용자 3층 병합과
    출처 표시, 다이얼 둘, 인라인 스타일을 변수 채널로 회수, #712. settings store 26→27).
    수정: 같은 줄 이미지가 자동저장에 escape 로 굳던 파일 손상(#672),
    링크 인덱스를 `md::literal` 로 통일(#662·#674), rename 이 기억한 줄 번호를 믿던 문제(#673),
    export 구분자 짝짓기(#679·#692)·Pandoc 이미지 판정(#670), Windows 폴더 rename 4중 결함(#671)
    ‼️ **테마 레지스트리는 아직 비어 있다** — Browse Themes 는 빈 선반이고, 내보낸 `.zip` 을
    파일에서 설치하는 경로도 없다(둘 다 의도적, 릴리스 노트에 명시). 테마 사용자 문서는 0줄
