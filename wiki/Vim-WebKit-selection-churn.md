...
재발 방지의 핵심은 이 절이다. 세 시도 모두 그럴듯했다 — 각각이 무엇으로 무너졌는지는 아래에 적었다.
...
2. **클릭 핸들러** (#408). 클릭으로 생기는 NodeSelection 은 PM 의 pointer dispatch 라
   `dispatchCursor` 를 타지 않는다 — vim 이 억제를 걸 자리가 없었다. microtask 에서
...
그래서 억제는 **결과가 아니라 호출로** 고정된다 — 되돌림 자체를 jsdom 이 만들지 못하니까.
같은 파일의 DOM range 제거는 **결과로** 핀하고 positive control 둘(visual 유지 · vim off 유지)을
붙였다. 무엇이 재현 가능한가가 그 선을 갈랐다.

- 억제 헬퍼를 스파이로 감싸 **호출됐는지를 단언한다.** 호출 여부까지다 — 억제를 focus 보다
  먼저 무장해야 한다는 위 4번의 순서 계약은 **어떤 핀도 잡고 있지 않고 코드 주석이 지킨다**
  (`use-source-mode.ts` 양 브랜치). 순서를 뒤집어도 스위트는 초록이다 — **핀을 추가하면 이
  문장을 고칠 것.**
