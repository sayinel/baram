...
core 는 ProseMirror 를 모른다. **정적 import 문에 PM 이 없고, purity 핀이 core 디렉터리의
`.ts` 를 훑어 그것을 고정한다** — 게이트 없는 경계는 첫 번째 편리한 import 에서 무너지기
때문이다. (핀이 보는 것은 `from "…"` 형태다. 동적 `await import()` 나 leaf 를 거친 전이 의존은
그 검사가 못 본다.)
...
있는 유일한 키 경로다. 이건 현재 prosemirror-view 의 입력 배선(`initInput` — custom 핸들러 먼저,
editable 게이트 나중)에 기댄 것이라 **업그레이드 때 확인할 계약이다.**

insert 모드만 `handleKeyDown` 을 쓴다 — PM 의 composition 전처리를 물려받아야 하기 때문이다.
거기로 들어오는 것은 Esc(그리고 그 Esc 를 transient 에게 양보하는 중재)와 코드블록 경계의
화살표 진입(#477)이다.

## 경계 4: 행 모델 — "줄" 을 정의하는 층

vim 의 세로 이동과 줄 오퍼레이션은 "줄" 을 전제한다. PM 문서는 트리이고 줄이 없다. 그 전제를
공급하는 것이 행 모델이고, 그래서 세로 이동과 줄 오퍼레이션이 "무엇이 한 줄인가" 를 각자
다시 정하지 않는다.
...
오프셋으로 흘려보내므로, 코드블록 착지는 일반 칼럼 산술에서 떼어 낸다. normal 모드의 기본은
**방향성 착지**다 — 진입 방향의 소스 줄(위에서 오면 첫 줄, 아래에서 오면 마지막 줄)에 캐리 칼럼
그대로 내려놓는다. 콘텐츠 시작으로 떨어지는 것은 visual 모드와 CM 섬이 없는 `journal-*` 블록뿐이다.

그 판정은 **노드 이름 매칭이다** — `spec.code` 로 판정하면 frontmatter 도 true 라서 회귀를
만들었다. 블록 **안에서의** 이동은 행 모델이 아니라 CM island 소관이다.
