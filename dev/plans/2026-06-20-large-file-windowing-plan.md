...
**Tech Stack:** Tiptap/ProseMirror v2 (`@tiptap/core`, `@tiptap/pm/{model,state,view}`), React 19, Vitest, TypeScript strict. Design spec: [`dev/plans/2026-06-20-large-file-windowing-design.md`](./2026-06-20-large-file-windowing-design.md).
...
Append the result to `dev/impl-notes/large-file-perf-c4-handoff.md` (a one-line "Step-0: scrollTop is LAYOUT/VISUAL space; measure with X" note). No commit needed if only the handoff changed; if you do edit the handoff, commit:

```bash
git add dev/impl-notes/large-file-perf-c4-handoff.md
...
```bash
git add src/extensions/plugins/viewport-virtualize.ts dev/impl-notes/large-file-perf-c4-handoff.md
...
Append a results block to `dev/impl-notes/large-file-perf-c4-handoff.md`: bench median, `inputLatency` p50, checklist pass/fail. If a check fails, debug (don't guess — trace with evidence) and loop back to the relevant task. If all pass:
