...
for (const { dir, expected } of GENERATED) {
  const actual = readdirSync(dir).filter((f) => !OS_ARTIFACTS.has(f));
  ...
  const tracked = new Set(
    execFileSync("git", ["ls-files", dir], { encoding: "utf-8" })
      .split("\n")
      .filter(Boolean)
      .map((p) => p.split("/").pop()),
  );
  ...
  if (unexpected.length > 0 || missing.length > 0 || untracked.length > 0) {
    ...
    if (unexpected.length > 0)
      console.error(`Unexpected files in ${dir}/: ${unexpected.join(", ")}`);
    if (missing.length > 0)
      console.error(`Missing generated files in ${dir}/: ${missing.join(", ")}`);
    if (untracked.length > 0)
      console.error(
        `Generated files present but not tracked by git in ${dir}/ (deleted from the index?): ${untracked.join(", ")}`,
      );
  } else {
    console.log(
      `${dir}/ file set OK (${expected.size} expected files, all tracked, nothing else).`,
    );
  }
}
