...
pub(super) async fn owning_contexts(ctx_mgr: &ContextManager, path: &str) -> Vec<ContextInfo> {
    ctx_mgr.contexts_containing(path).await
}

/// The index keys of `contexts`: their registered paths.
pub(super) fn keys_of(contexts: &[ContextInfo]) -> Vec<String> {
...
pub(super) fn buildable(contexts: &[ContextInfo]) -> Vec<ContextInfo> {
    contexts
        .iter()
