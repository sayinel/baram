...
/// `{tasks_home}/tasks/` 아래(그리고 수집함 파일)뿐이다.
///
/// `capture_path`는 프런트가 이미 해석한 **절대 경로**다(`resolveCapturePath`). Rust는 그것을
/// 다시 만들지 않고 태스크 홈 안·마크다운인지만 확인한다 — 해석기를 두 벌 두면 갈라지지만
/// 부분집합 단정은 갈라질 수 없다.
///
...
pub async fn archive_task_lines(
    ...
    crate::task::archive_tasks(&tasks_home, &capture_path, &items, &today, after_days)
        .await
        .map_err(|e| e.to_string())
}
