 ...
 * 있는 **pretty-printed** 파일의 **바이트** 수를 잰다(`std::fs::metadata`). 한글처럼
 ...
 *
 * ‼️ 그리고 이 필드별 검사(`validateTextField`)는 `name`·`description` 둘에만 걸린다
 * (아래 135–136번째 줄). `author`·`license`·`version`은 이 함수 안에서 "빈 문자열이
 ...
 * 검증기이므로(`install.rs:840` 부근이 이 검증을 재구현하지 않고 위임한다), 세 필드를
