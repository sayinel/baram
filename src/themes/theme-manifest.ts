...
export interface ThemeManifest {
   ...
   * (§366 의 층 순서).
   *
   * ‼️ 모드별이 아니라 최상위다. 모드에 의존하는 값은 **저장된 다이얼 값이 아니라
   * 파생**에 산다 — 강조색 다이얼(§367)은 색상과 채도만 저장하고 명도는 지금 입은
   * 팔레트의 강조색에서 가져오므로, 값 하나가 모드마다 다른 hex 를 낸다. 테마가
   * 모드마다 다른 강조를 원하면 `modes.{light,dark}.colors` 가 이미 그 자리다.
   * 계획 0095 가 `modes.{light,dark}.dials` 를 **짓지 않기로** 판단한 근거가 그것이다.
   *
