 ...
 *
 * `v0.7.4`가 그 버전이다 — 테마 설치 경로(§360)와 이 내보내기(§363)를 처음 싣고 나가는
 * 릴리스다. 전 판까지 이 상수는 `">=0.0.0"`이었고 그것은 하한 주장이 아니라 **자리표시자**
 * 였다(0091 fix round 1, Finding 3): 그때는 이 포맷을 설치할 수 있는 릴리스가 하나도
 * 없었으므로 `">=0.0.0"`("릴리스된 모든 Baram이 설치할 수 있다")은 사실과 정반대였다.
 * 그 주석이 "§360이 실제로 릴리스되는 날 그 버전 번호로 바꿀 것"이라고 적어 두었고,
 * v0.7.4 릴리스 준비가 그날이다.
 *
 * 실측(v0.7.4 범프 시점, `docs/v0.7.4-release-docs` 브랜치): `git merge-base --is-ancestor
 * e07eeb44 v0.7.3`이 **거짓**이고 `… origin/main`이 **참** — §360을 처음 들여온 커밋은
 * v0.7.3에 없고 main에는 있다. 즉 v0.7.4가 이 포맷의 첫 릴리스라는 것이 태그 사실로 참이다.
 *
 * (이 함수가 순수해서 `getVersion()`을 부를 수 없다는 것은 이 필드가 상수인 이유가 아니다
 * — 두 판 전의 이 주석이 그렇게 적었고 그것은 비약이었다. floor는 **런타임 값이 아니라
 * 릴리스 사실**이라 상수로 두는 것 자체가 항상 순수하다.)
 *
 * ‼️ 문법도 값만큼 중요하다 — `validateThemeManifest`는 "비어있지 않은 문자열"만 요구하고
 * 문법을 모른다. 설치 시점에 이 값을 읽는 것은 `unmetFloorAgainstApp`의 `parseBaramFloor`
 * 이고, 파싱 불가한 값(`">= v0.7.4"`의 공백·`v` 접두처럼)은 "의견 없음"으로 **조용히**
 * 읽혀 이 필드가 아예 없는 것과 똑같이 동작하면서도 있는 것처럼 보인다. 그래서
 * `theme-package-export.test.ts`가 값이 아니라 `parseBaramFloor(...) !== null`을 고정한다.
 *
 * ‼️ 이 상수를 바꿀 때 `src-tauri/src/plugin/fixtures/theme-package.json`의
 * `expectedManifest.engines.baram`도 같은 문자열을 박아 두므로 **함께** 바꿔야 한다 —
 * 안 그러면 그 fixture 교차-언어 테스트가 (옛 값을 기대하며) 실패한다.
 */
const MIN_BARAM_FOR_TOKENS_PACKAGE = ">=0.7.4";
