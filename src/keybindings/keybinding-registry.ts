 ...
 * 하나다. 문맥이 무엇을 하는지 정한다: 캡처창이 열려 있으면 태스크 모드 토글(그쪽
 * `handleKeyDown`이 직접 처리한다), 에디터면 편집 모달. 명령을 둘로 두면 같은 키를 두
 * 항목이 노려 충돌 검사에 걸리고, 사용자가 한쪽만 리바인딩해 두 곳의 뜻이 갈라질 수 있다.
 *
 * 상수로 두는 이유: 문자열을 호출자에 직접 적으면 매직 스트링이 두 벌이 된다.
 */
