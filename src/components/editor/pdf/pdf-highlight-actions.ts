 ...
 *
 * 이 경로가 필요한 이유는 아카이브가 무한히 자라면 안 되기 때문이다.
 *
 * ‼️ **민감한 내용을 치우는 수단은 아니다.** 동반 노트의 문단은 건드리지 않고
 * (§274가 그은 경계 — 그 문단이 원문의 마지막 사본이고, 사용자 노트를 앱이
 * 지우는 것은 별개의 결정이다), 사이드카에는 좌표와 색만 있다(§273.2). 즉
 * 지워지는 것은 위치이고 **텍스트는 그대로 남는다.** 확인 문구가 이 사실을
 * 명시한다 — 호출부가 이것을 사용자에게 숨기지 않아야 한다.
 */
