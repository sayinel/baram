...
//
// ‼️ 이 합류도 사이드카 쪽과 **같은 구조적 보장**을 받는다 — 단, 그 보장은
// 이 파일이 스스로 만드는 것이 아니라 사이드카 합류에서 물려받는 것이다.
// 동반 노트 읽기는 `await readSidecarCoalesced(...)` 뒤에 시작되고, 그 사이에는
// 실제 지연이 있는 await가 없다. `companion`은 사이드카 경로와 1:1이므로 같은
// 동반 노트를 공유하는 참조들은 **같은 사이드카 Promise 하나**를 기다리고,
// 그것이 settle되면 같은 마이크로태스크 드레인에서 함께 재개된다. 첫 동반 노트
// 읽기의 실제 I/O는 매크로태스크라 그 드레인이 끝난 뒤에야 완료될 수 있다 —
// 그래서 N건이 전부 합류한다.
//
// 이 전파는 §276.5가 사이드카 합류를 함께 넣었기 때문에 성립한다. 사이드카를
// 참조마다 따로 읽던 동안에는 완료 시점이 흩어져 동반 노트 읽기가 새어
// 나갔다(재리뷰 대조 프로브: 5건 → 2회). 사이드카 합류를 되돌리면 이 보장도
// 같이 사라진다.
