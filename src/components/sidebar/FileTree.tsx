...
export function FileTree(): React.JSX.Element {
  ...
  const handleFileClick = useCallback(
    async (entry: FileEntry, e: React.MouseEvent): Promise<void> => {
      ...
      try {
        // PDFs/images are binary — never read through the UTF-8 IPC; the
        // viewer loads them via the asset: protocol. Cache "" so tab
        // switching treats the tab as loaded.
        const content = isBinaryViewerFile(entry.path)
          ? ""
