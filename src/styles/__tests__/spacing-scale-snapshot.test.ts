...
// 리뷰 표면이다.
//
// ‼️ 셀렉터 이름을 바꾸거나 규칙을 **다른 파일로** 옮겨도 실패한다. 이 스냅샷은 값만이
// 아니라 (파일·셀렉터·속성) 자리까지 고정한다 — 리팩터 중에 규칙이 사라지는 것도 결함이다.
// 줄 번호와 `@media` 조건은 그 자리에 없다: 같은 파일 안에서 규칙을 옮기거나, 기본 규칙과
// 그 `@media` 쌍둥이가 값을 맞바꾸면 정렬된 배열이 그대로라 통과한다(2026-09-23 에는
// `@media` 안에 간격·모서리 선언이 하나도 없어 뒤쪽은 아직 비어 있는 구멍이다).
