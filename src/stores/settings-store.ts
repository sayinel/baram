  ...
  persist(
    (set) => ({
      ...
      resetActivityBarConfig: () =>
        set({ activityBarConfig: DEFAULT_ACTIVITY_BAR_CONFIG }),
      ...
      setZoomLevel: (zoomLevel) =>
        set({
          zoomLevel:
            Math.round(Math.max(0.5, Math.min(2, zoomLevel)) * 100) / 100,
        }),
      ...
      setJournalTemplatePath: (journalTemplatePath) =>
        set({ journalTemplatePath }),
      setJournalStartupBehavior: (journalStartupBehavior) =>
        set({ journalStartupBehavior }),
      setJournalUseHierarchy: (journalUseHierarchy) =>
        set({ journalUseHierarchy }),
      ...
      // §56e Mood/Energy setters
      setJournalMoodEnabled: (journalMoodEnabled) =>
        set({ journalMoodEnabled }),
      ...
      setJournalCustomThemes: (journalCustomThemes) =>
        set({ journalCustomThemes }),
