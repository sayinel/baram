...
function createDeniedProxy(apiName: string, requiredCapability: PluginCapability): unknown {
  return new Proxy({}, {
    get(_target, prop) {
...
function createCommandsAPI(pluginId: string, disposables: Disposable[]): CommandsAPI {
  return {
    register(id: string, handler: (...args: unknown[]) => unknown): Disposable {
      const fullId = `${pluginId}.${id}`;
      commandHandlers.set(fullId, handler);
      const disposable: Disposable = {
        dispose: () => { commandHandlers.delete(fullId); },
      };
      disposables.push(disposable);
      return disposable;
    },
    async execute(id: string, ...args: unknown[]): Promise<unknown> {
      const handler = commandHandlers.get(id) ?? commandHandlers.get(`${pluginId}.${id}`);
...
function createEventsAPI(disposables: Disposable[]): EventsAPI {
  return {
    on(event: string, handler: EventHandler): Disposable {
      ...
      const disposable: Disposable = {
        dispose: () => { eventListeners.get(event)?.delete(handler); },
      };
      disposables.push(disposable);
      return disposable;
    },
    emit(event: string, ...args: unknown[]): void {
      eventListeners.get(event)?.forEach((handler) => {
        try { handler(...args); } catch (e) { console.error(`[Plugin Event Error] ${event}:`, e); }
      });
    },
  };
}
...
// --- Editor API ---
let editorInstance: { getHTML: () => string; getText: () => string; commands: Record<string, unknown>; state: { selection: { from: number; to: number } }; chain: () => Record<string, unknown> } | null = null;
...
function createEditorAPI(readonly: boolean): EditorAPI {
  return {
    ...
    setContent(content: string): void {
      if (readonly) throw new Error("editor:readonly — setContent is not allowed");
      if (!editorInstance) return;
      (editorInstance.commands as Record<string, (c: { content: string }) => void>).setContent({ content });
    },
    ...
    insertText(text: string): void {
      if (readonly) throw new Error("editor:readonly — insertText is not allowed");
      if (!editorInstance) return;
      (editorInstance.commands as Record<string, (t: string) => void>).insertContent(text);
    },
  };
}
...
function createFilesAPI(readonly: boolean): FilesAPI {
  return {
    ...
    async writeFile(path: string, content: string): Promise<void> {
      if (readonly) throw new Error("files:readonly — writeFile is not allowed");
...
function createUIAPI(disposables: Disposable[]): UIAPI {
  return {
    ...
    showStatusBarItem(text: string, _alignment: "left" | "right" = "right"): Disposable {
...
export function createExtensionContext(
  ...
  const ui: UIAPI = (hasCapability("sidebar") || hasCapability("statusbar"))
...
export async function executePluginCommand(id: string, ...args: unknown[]): Promise<unknown> {
