...
export function validateManifest(data: unknown): { valid: true; manifest: PluginManifest } | { valid: false; errors: ManifestValidationError[] } {
  ...
  for (const field of ["id", "name", "description", "version", "author", "license", "main"]) {
  ...
  if (typeof obj.id === "string" && !/^[a-z0-9-]+$/.test(obj.id)) {
    errors.push({ field: "id", message: "id must contain only lowercase letters, digits, and hyphens" });
  }
  ...
  if (!obj.engines || typeof obj.engines !== "object") {
    ...
    if (!engines.baram || typeof engines.baram !== "string") {
      errors.push({ field: "engines.baram", message: "engines.baram version is required" });
    }
  }
  ...
  if (obj.tiptapExtensions !== undefined) {
    if (!Array.isArray(obj.tiptapExtensions)) {
      errors.push({ field: "tiptapExtensions", message: "tiptapExtensions must be an array" });
    } else {
      for (let i = 0; i < obj.tiptapExtensions.length; i++) {
        ...
        if (!["node", "mark", "plugin"].includes(ext.type as string)) {
          errors.push({ field: `tiptapExtensions[${i}].type`, message: "type must be node, mark, or plugin" });
        }
        if (!ext.name || typeof ext.name !== "string") {
          errors.push({ field: `tiptapExtensions[${i}].name`, message: "name is required" });
        }
        if (!ext.exportName || typeof ext.exportName !== "string") {
          errors.push({ field: `tiptapExtensions[${i}].exportName`, message: "exportName is required" });
        }
      }
    }
  }
