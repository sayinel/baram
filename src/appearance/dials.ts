 ...
 * 시작한다(실측: 16px 에서 21.43px 대 11.34px, 0.63em 차이). 이 두 거리는 그때의 화살표인
 * 0.375em 테두리 삼각형이 접혀 오른쪽을 가리킬 때까지의 거리다. 그것을 대신한 chevron 은
 * 같은 방향에서 0.125em 좁게 가운데에 칠하므로 16px 에서 각각 1px 더 크다 — 유도한 값이고
 * 다시 재지 않았다. 마커 위치는 그대로라 0.63em 차이도 그대로다. 숫자 기준은 시작점을
