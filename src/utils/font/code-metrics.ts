...
//
// 왜 계산을 여기 하나로 모으는가: 이 함수를 쓰는 곳(에디터 효과 `use-settings-effects.ts` ·
// 설정 탭의 코드 두 행 `EditorTab.tsx` · 브라우저 미리보기 `font-browser-preview.tsx` · 소스
// 모드 `SourceCodeEditor.tsx` — 테스트 밖 `resolveCodeMetrics` 호출 전부, 2026-09-26 grep)이 같은
// 답을 내야 한다. 비율을 각자 적으면 한 곳만 고쳐도 아무 테스트가 깨지지 않은 채 화면마다 다른
// 크기를 보여 준다.
