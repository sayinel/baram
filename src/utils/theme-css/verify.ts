...
//      ‼️ 계약 3 과 **같은 구조의 구멍이 여기에도 있고, 그대로 둔다.** 중첩 두 겹부터
//      `Raw` 가 삼킨다(실측: `html{@import "x";}` 는 거부, `html{body{@import "x";}}` 와
//      세 겹은 **통과**). 그런데도 토큰 스캔으로 옮기지 않는 이유는 파싱이 아니라
//      스펙이다 — `@import` 는 `@layer name{}` 블록 **안**에서는 중첩 깊이와 무관하게
//      무효이고 브라우저가 버린다(Cascade Layers). 계약 1 이 최상위를 전부 그 블록으로
//      강제하므로, `Raw` 에 숨은 `@import` 는 우리가 못 볼 뿐 아무것도 끌어오지 못한다.
//      계약 3 이 이와 달랐던 것은 layered `!important` 가 **유효**하기 때문이다
//
