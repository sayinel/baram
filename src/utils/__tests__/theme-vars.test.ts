...
//
// ‼️ 그래서 **이름이 계약이다.** 그 단언은 `startsWith("apply")` 로만 훑으므로
// `attachThemeFont`·`injectThemeSheet` 처럼 다르게 이름 붙인 적용물은 잡지 못하고,
// `clear*` 쪽은 열거하지도 않는다. 남는 구멍이 좁은 것은 아래 왕복 스냅샷이 **불린
// 함수가 한 일**이라면 무엇이든 보기 때문이지 이 단언이 넓어서가 아니다. 이 모듈에
// 적용물을 더하는 사람은 `apply…`/`clear…` 로 이름 짓고 왕복에 끼워 넣을 것.
