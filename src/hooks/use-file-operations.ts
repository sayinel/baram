...
export async function triggerAutoReload(
  ...
  // PDFs/images are binary — keep the "" cache sentinel; the mtime bump
  // below refreshes the viewer instead.
...
export function useFileOperations({
  ...
  const handleOpenFile = useCallback(async () => {
    ...
    try {
      // PDFs/images are binary — never read through the UTF-8 IPC (viewer
      // loads them via asset:). Cache "" so tab switching treats the tab
      // as loaded.
  ...
  const handleSave = useCallback(async () => {
    ...
    // PDF/image tabs are read-only viewers — writing sourceContentRef (which
    // holds another tab's text) into the binary would destroy it.
  ...
  const handleSaveAs = useCallback(async () => {
    ...
    // PDF/image tabs are read-only viewers — Save As would write text,
    // not the binary.
