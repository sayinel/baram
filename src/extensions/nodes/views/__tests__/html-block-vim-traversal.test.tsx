// §298 §12-⑩ — the html block adopts the math block's entry model.
//
// Same disease, same organ: the render branch was `if (!selected)`, so a vim
// traversal NodeSelection opened the editing chrome (header, textarea) — the
// device finding fixed for math in f12e2af0 and ported to mermaid and svg.
// Like those, html had no click bypass and no Esc stair.
//
...
const DOC: JSONContent = {
  content: [
    ...
    { attrs: { content: "<div>hello</div>" }, type: "htmlBlock" },
...
function wrapper(): HTMLElement {
  const el = document.querySelector<HTMLElement>(".html-block");
...
describe("vim modal: selection alone keeps the preview render", () => {
  it("a traversal NodeSelection does NOT open the editing UI", async () => {
    ...
    expect(wrapper().className).toContain("html-block-preview");
    expect(wrapper().className).not.toContain("html-block-editing");
  });
  ...
  it("a click while modal only SELECTS the block — `i` is the entry", async () => {
    ...
    expect(editor.state.selection.from).toBe(htmlPos(editor));
    expect(wrapper().className).toContain("html-block-preview");
    expect(wrapper().className).not.toContain("html-block-editing");
  });
  ...
  it("focus arriving in the standby textarea opens the editing UI", async () => {
    ...
    expect(wrapper().className).toContain("html-block-editing");
